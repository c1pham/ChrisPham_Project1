...
def test_save_all_jobs_to_db(get_data):
    db_connection, db_cursor = GatherJobs.open_db("all_jobs_db")  # open db
    ...
    db_connection, db_cursor = GatherJobs.open_db("all_jobs_db")
...
def test_save_specific_job_to_db_bad_data():
    db_connection, db_cursor = GatherJobs.open_db("jobs_save_bad_data_db")  # open db
    GatherJobs.create_github_jobs_table(db_cursor)
    ...
    GatherJobs.add_job_to_db(db_cursor, all_jobs[0])
    GatherJobs.close_db(db_connection)

    db_connection, db_cursor = GatherJobs.open_db("jobs_save_bad_data_db")
    ...
    GatherJobs.close_db(db_connection)
    assert num_results_after_saving - num_results_before_saving == 0
